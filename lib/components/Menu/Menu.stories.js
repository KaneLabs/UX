var _interopRequireDefault = require('@babel/runtime/helpers/interopRequireDefault');
Object.defineProperty(exports, '__esModule', { value: true });
exports.default = void 0;
var _react = _interopRequireDefault(require('react'));
var _View = _interopRequireDefault(
  require('react-native-web/dist/exports/View'),
);
var _storybookState = require('@sambego/storybook-state');
var _ = require('../..');
var _Header = _interopRequireDefault(
  require('../../storybook/components/Header'),
);
var _Container = _interopRequireDefault(
  require('../../storybook/components/Container'),
);
var _storiesOf = require('eros-ui-storybook/helpers/storiesOf');
var _this = this,
  _jsxFileName = '/Users/kane/Lab/Eros/src/components/Menu/Menu.stories.js';
var store = new _storybookState.Store({
  visibleOne: false,
  visibleTwo: false,
  visibleThree: false,
  visibleFour: false,
  visibleFive: false,
});
var _default = (0, _storiesOf.storiesOf)('Components|Menu', module)
  .addParameters({ jest: ['Menu'] })
  .add('Simple', function() {
    return _react.default.createElement(
      _Container.default,
      {
        __self: _this,
        __source: { fileName: _jsxFileName, lineNumber: 23, columnNumber: 5 },
      },
      _react.default.createElement(_Header.default, {
        title: 'Menu',
        __self: _this,
        __source: { fileName: _jsxFileName, lineNumber: 24, columnNumber: 7 },
      }),
      _react.default.createElement(
        _.Heading,
        {
          style: { marginBottom: 10 },
          __self: _this,
          __source: { fileName: _jsxFileName, lineNumber: 25, columnNumber: 7 },
        },
        'On the left',
      ),
      _react.default.createElement(
        _storybookState.State,
        {
          store: store,
          style: { flex: 1 },
          __self: _this,
          __source: { fileName: _jsxFileName, lineNumber: 26, columnNumber: 7 },
        },
        function(state) {
          return _react.default.createElement(
            _View.default,
            {
              style: { marginBottom: 40 },
              __self: _this,
              __source: {
                fileName: _jsxFileName,
                lineNumber: 28,
                columnNumber: 11,
              },
            },
            _react.default.createElement(
              _.Menu,
              {
                menuStyle: { width: 324 },
                visible: state.visibleOne,
                onBackdropPress: function onBackdropPress() {
                  store.set({ visibleOne: false });
                },
                button: _react.default.createElement(_.Button, {
                  text: 'Show menu',
                  onPress: function onPress() {
                    store.set({ visibleOne: !state.visibleOne });
                  },
                  type: 'contained',
                  __self: _this,
                  __source: {
                    fileName: _jsxFileName,
                    lineNumber: 36,
                    columnNumber: 17,
                  },
                }),
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 29,
                  columnNumber: 13,
                },
              },
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 1',
                onPress: function onPress() {
                  return store.set({ visibleOne: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 45,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 2',
                onPress: function onPress() {
                  return store.set({ visibleOne: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 46,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 3',
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleOne: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 47,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 4',
                onPress: function onPress() {
                  return store.set({ visibleOne: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 52,
                  columnNumber: 15,
                },
              }),
            ),
          );
        },
      ),
      _react.default.createElement(
        _storybookState.State,
        {
          store: store,
          style: { flex: 1 },
          __self: _this,
          __source: { fileName: _jsxFileName, lineNumber: 57, columnNumber: 7 },
        },
        function(state) {
          return _react.default.createElement(
            _View.default,
            {
              __self: _this,
              __source: {
                fileName: _jsxFileName,
                lineNumber: 59,
                columnNumber: 11,
              },
            },
            _react.default.createElement(
              _.Menu,
              {
                menuStyle: { width: 324 },
                visible: state.visibleTwo,
                onBackdropPress: function onBackdropPress() {
                  store.set({ visibleTwo: false });
                },
                button: _react.default.createElement(_.IconButton, {
                  name: 'more-vert',
                  onPress: function onPress() {
                    store.set({ visibleTwo: !state.visibleTwo });
                  },
                  size: 24,
                  __self: _this,
                  __source: {
                    fileName: _jsxFileName,
                    lineNumber: 67,
                    columnNumber: 17,
                  },
                }),
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 60,
                  columnNumber: 13,
                },
              },
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 1',
                onPress: function onPress() {
                  return store.set({ visibleTwo: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 76,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 2',
                onPress: function onPress() {
                  return store.set({ visibleTwo: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 77,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 3',
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleTwo: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 78,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 4',
                onPress: function onPress() {
                  return store.set({ visibleTwo: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 83,
                  columnNumber: 15,
                },
              }),
            ),
          );
        },
      ),
      _react.default.createElement(
        _.Heading,
        {
          style: { marginBottom: 10, marginTop: 50, alignSelf: 'flex-end' },
          __self: _this,
          __source: { fileName: _jsxFileName, lineNumber: 88, columnNumber: 7 },
        },
        'On the Right',
      ),
      _react.default.createElement(
        _storybookState.State,
        {
          store: store,
          style: { flex: 1 },
          __self: _this,
          __source: { fileName: _jsxFileName, lineNumber: 91, columnNumber: 7 },
        },
        function(state) {
          return _react.default.createElement(
            _View.default,
            {
              style: { alignItems: 'flex-end', marginBottom: 40 },
              __self: _this,
              __source: {
                fileName: _jsxFileName,
                lineNumber: 93,
                columnNumber: 11,
              },
            },
            _react.default.createElement(
              _.Menu,
              {
                visible: state.visibleThree,
                onBackdropPress: function onBackdropPress() {
                  store.set({ visibleThree: false });
                },
                sameWidth: true,
                button: _react.default.createElement(_.Button, {
                  text: 'Show menu',
                  onPress: function onPress() {
                    store.set({ visibleThree: !state.visibleThree });
                  },
                  type: 'contained',
                  __self: _this,
                  __source: {
                    fileName: _jsxFileName,
                    lineNumber: 101,
                    columnNumber: 17,
                  },
                }),
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 94,
                  columnNumber: 13,
                },
              },
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 1',
                style: { paddingHorizontal: '12%' },
                onPress: function onPress() {
                  return store.set({ visibleThree: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 110,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 2',
                style: { paddingHorizontal: '12%' },
                onPress: function onPress() {
                  return store.set({ visibleThree: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 115,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 3',
                style: { paddingHorizontal: '12%' },
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleThree: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 120,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 4',
                style: { paddingHorizontal: '12%' },
                onPress: function onPress() {
                  return store.set({ visibleThree: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 126,
                  columnNumber: 15,
                },
              }),
            ),
          );
        },
      ),
      _react.default.createElement(
        _storybookState.State,
        {
          store: store,
          style: { flex: 1 },
          __self: _this,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 135,
            columnNumber: 7,
          },
        },
        function(state) {
          return _react.default.createElement(
            _View.default,
            {
              style: { alignItems: 'flex-end' },
              __self: _this,
              __source: {
                fileName: _jsxFileName,
                lineNumber: 137,
                columnNumber: 11,
              },
            },
            _react.default.createElement(
              _.Menu,
              {
                visible: state.visibleFour,
                onBackdropPress: function onBackdropPress() {
                  store.set({ visibleFour: false });
                },
                button: _react.default.createElement(_.IconButton, {
                  name: 'more-vert',
                  onPress: function onPress() {
                    store.set({ visibleFour: !state.visibleFour });
                  },
                  size: 24,
                  __self: _this,
                  __source: {
                    fileName: _jsxFileName,
                    lineNumber: 144,
                    columnNumber: 17,
                  },
                }),
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 138,
                  columnNumber: 13,
                },
              },
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 1',
                onPress: function onPress() {
                  return store.set({ visibleFour: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 153,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 2',
                onPress: function onPress() {
                  return store.set({ visibleFour: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 154,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 3',
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleFour: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 155,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 4',
                onPress: function onPress() {
                  return store.set({ visibleFour: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 160,
                  columnNumber: 15,
                },
              }),
            ),
          );
        },
      ),
    );
  })
  .add('Scrolling', function() {
    return _react.default.createElement(
      _Container.default,
      {
        __self: _this,
        __source: { fileName: _jsxFileName, lineNumber: 168, columnNumber: 5 },
      },
      _react.default.createElement(_Header.default, {
        title: 'Menu',
        __self: _this,
        __source: { fileName: _jsxFileName, lineNumber: 169, columnNumber: 7 },
      }),
      _react.default.createElement(
        _storybookState.State,
        {
          store: store,
          style: { flex: 1 },
          __self: _this,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 171,
            columnNumber: 7,
          },
        },
        function(state) {
          return _react.default.createElement(
            _View.default,
            {
              style: { marginBottom: 40 },
              __self: _this,
              __source: {
                fileName: _jsxFileName,
                lineNumber: 173,
                columnNumber: 11,
              },
            },
            _react.default.createElement(
              _.Menu,
              {
                menuStyle: { width: 324, maxHeight: 200 },
                visible: state.visibleFive,
                onBackdropPress: function onBackdropPress() {
                  store.set({ visibleFive: false });
                },
                button: _react.default.createElement(_.Button, {
                  text: 'Show menu',
                  onPress: function onPress() {
                    store.set({ visibleFive: !state.visibleFive });
                  },
                  type: 'contained',
                  __self: _this,
                  __source: {
                    fileName: _jsxFileName,
                    lineNumber: 181,
                    columnNumber: 17,
                  },
                }),
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 174,
                  columnNumber: 13,
                },
              },
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 1',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 190,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 2',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 191,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 3',
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 192,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 4',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 197,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 5',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 198,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 6',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 199,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 7',
                disabled: true,
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 200,
                  columnNumber: 15,
                },
              }),
              _react.default.createElement(_.MenuItem, {
                text: 'Menu Item 8',
                onPress: function onPress() {
                  return store.set({ visibleFive: false });
                },
                __self: _this,
                __source: {
                  fileName: _jsxFileName,
                  lineNumber: 205,
                  columnNumber: 15,
                },
              }),
            ),
          );
        },
      ),
    );
  });
exports.default = _default;
